...
def get_dataloader(ds, pad_to_longest = True, **kwargs):
    ...
    return DataLoader(ds, collate_fn = collate_fn, num_workers=4, **kwargs)
...
class MuLaNTrainer(nn.Module):
    ...
    def train_step(self):
        ...
        # save model for the minimum loss, add min_loss value to the file name
        if logs['loss'] < self.min_loss:
            self.min_loss = logs['loss']
            model_path = str(self.results_folder / 'mulan_min_loss.pt')
            self.save(model_path)
            self.print(f'{steps}: saving model with minimum loss to {str(self.results_folder)}')
