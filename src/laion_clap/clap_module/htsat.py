...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 i_layer=0, i_layer_depth=0, extract_head_features=True):
        ...
        self.i_layer_depth = i_layer_depth
        self.extract_head_features = extract_head_features
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            i_layer=i_layer, i_layer_depth=i_layer_depth, extract_head_features=extract_head_features)
        # For Decomposition
        self.i_layer = i_layer
        self.i_layer_depth = i_layer_depth
        self.extract_head_features = extract_head_features
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        extract_flag = self.i_layer == 3 and self.i_layer_depth == 1
        if extract_flag and self.extract_head_features:
            attn_windows, attn, head_features = self.attn(x_windows, mask=self.attn_mask)
        else:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 norm_before_mlp='ln', i_layer=0, extract_head_features=True):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 i_layer=i_layer, i_layer_depth=i)
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                ...
                if self.extract_head_features and self.i_layer == 3 and i == 1:
                    x, attn, head_features = blk(x)
                else:
                    x, attn = blk(x)
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
                in_chans=1, num_classes=527,
                 ...
                 enable_fusion = False, fusion_type = 'None', extract_head_features=True,
                 **kwargs):
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(self.embed_dim * 2 ** i_layer),
                ...
                i_layer=i_layer,
                extract_head_features=extract_head_features)
    ...
    def forward_features(self, x, longer_idx = None):
        ...
        for i, layer in enumerate(self.layers):
            if i == 3 and self.extract_head_features:
                x, attn, head_features = layer(x)
            else:
                x, attn = layer(x)
        # for x
        x = self.norm(x)
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False, device=None):# out_feat_keys: List[str] = None):
        ...
        if not self.enable_fusion:
            ...
            x = self.reshape_wav2img(x) # change to the images
